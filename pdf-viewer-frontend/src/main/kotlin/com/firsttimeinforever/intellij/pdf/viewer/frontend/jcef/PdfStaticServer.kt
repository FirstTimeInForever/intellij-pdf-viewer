...
internal class PdfStaticServer : HttpRequestHandler() {
  ...
  private fun sendExternalFile(url: String, context: ChannelHandlerContext, request: FullHttpRequest) {
    ...
    val localPath = try {
      if (file.isInLocalFileSystem) Paths.get(file.path) else null
    } catch (e: Exception) {
      null
    }

    if (localPath != null && Files.exists(localPath)) {
      logger.debug("Sending external file, path: $localPath")
      FileResponses.sendFile(request, channel, localPath)
    } else {
      logger.debug("Sending external file via stream, url: ${file.url}")
  ...
  fun getPreviewUrl(file: VirtualFile, withReloadSalt: Boolean = false): String {
    ...
    return server.addAuthToken(url)
      ...
      .addParameters(mapOf("__reloadSalt" to "$salt", "file" to "/$uuid/get-file/${URLUtil.encodeURIComponent(fileID)}"))
      .toExternalForm()
  }

  fun disposePreviewUrl(file: VirtualFile) {
    vfsMap.remove(file.uniqueID)
  }
  ...
  private val VirtualFile.uniqueID: String
    get() {
      if (this is LightVirtualFileBase) {
